...
// This sketch was originally created by Arik Yavilevich under GPL v3 license
// Modified by Anurag Purwar, Nov 8, 2024; Distributed under the same license
...
// misc
#define SERIAL_BAUD 9600      // changed from 115200 by A. Purwar
...
#define INITIAL_DELAY 1000    // in ms, increase it if initial serial print is missing

enum ModuleType { HM10,
                  CC41,
                  MLT_BT05,
                  Unknown };
enum Operation { Quit,
                 ...
                 SetRole,
                 SendCommand };
enum ConnectionState { NoStatePin,
                       Blinking,
                       Connected,
                       Disconnected };
...
void openBLE() {
  rxPin = readInt(F("Enter the number of the RX pin on the Arduino, TX on the module [press enter if using SnappyXO Motor shield header]"), RX_PIN);
  txPin = readInt(F("Enter the number of the TX pin on the Arduino, RX on the module [press enter if using SnappyXO Motor shield header]"), TX_PIN);
  ...
  statePin = -1;  // commented above line and this assignment manually added by A. Purwar
...
#pragma GCC diagnostic pop
  ...
  Serial2.begin(BLE_BAUD, SERIAL_8N1, rxPin, txPin);
  ble = &Serial2;
#endif
...
ModuleType identifyDevice() {
  ...
  Serial.println(F("No response received from module or unknown module type."));
